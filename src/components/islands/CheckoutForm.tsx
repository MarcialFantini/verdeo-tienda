...
import { formatPrecio, calcularEnvio, generarNumeroPedido, COSTO_GIFT_WRAP } from "../../lib/site";
import {
  aplicarCoupon,
  findCoupon,
  totalConCoupon,
...
import GiftWrapToggle from "./GiftWrapToggle.tsx";
...
export default function CheckoutForm() {
  ...
  // Costo del gift wrap impacta el envío (se computa aparte).
  const envioBase = calcularEnvio(subtotal);
  const envioBaseConGift = envioBase + (giftWrap ? COSTO_GIFT_WRAP : 0);
  const breakdown = aplicarCoupon(subtotal, envioBaseConGift, coupon);
  ...
  const handleApplyCoupon = (e: Event) => {
    ...
    const { error } = aplicarCoupon(subtotal, envioBaseConGift, found);
    if (error) {
      setCoupon(null);
      setCouponMsg({ tipo: "err", texto: error });
      return;
    }
    setCoupon(found);
    ...
    <form onSubmit={handleSubmit} novalidate>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
        ...
        <section class="lg:col-span-7 flex flex-col gap-8" aria-label="Datos del pedido">
          ...
          <div>
            <h2 class="display text-[28px] mb-5">¿Es un regalo?</h2>
            <GiftWrapToggle
              enabled={giftWrap}
              message={giftMessage}
              ...
              <dl class="flex flex-col gap-2 mt-5 text-sm">
                ...
                {giftWrap && (
                  <div class="flex items-center justify-between">
                    ...
                    <dd class="tabular-nums">{formatPrecio(COSTO_GIFT_WRAP)}</dd>
                  </div>
                )}
                <div class="flex items-center justify-between">
